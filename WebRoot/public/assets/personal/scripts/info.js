...
//	function getInfo(){
//		$.post("/other/getUserInfo",function(data){
//			techInfo = data;
//		},'json');
//	}
	//填充数据
	function filldata(data){
			...
			if(data!=""){
				...
				$("#Connect").val(data.othercontact==null?"":data.othercontact);
				$("#Website").val(data.ownurl==null?"":data.ownurl);
				$("#Address").val(data.address==null?"":data.address);
				$("#eAddress").val(data.eaddress);
				$("#ConnectMan").val(data.urgencynam==null?"":data.urgencynam);
				$("#eConnectMan").val(data.eurgencynam);
				$("#Profession").val(data.profession);
				$("#eProfession").val(data.eprofession);
				$("#Research").val(data.researcharea==null?"":data.researcharea);
				$("#eResearch").val(data.eresearcharea);
				$("#Tutor").val(data.tutor==null?"":data.tutor);
				$("#eTutor").val(data.etutor);
				$("#Thesis").val(data.thesis==null?"":data.thesis);
...
//				$("#epesnIntro").val(data.eintroduce);
				...
				editor.html(data.introduce==null?"":data.introduce);
				eeditor.html(data.eintroduce==null?"":data.eintroduce);
				wditor.html(data.workresume==null?"":data.workresume);
				wwditor.html(data.eworkresume==null?"":data.eworkresume);
			}
	};
	//显示个人信息
	function listinfo(){
...
//				$("#epesnIntro").html(data.eintroduce);
	...
	$("#b_save").live('click',function(){
		...
		$.post("/add/updateUserInfor", formdata, function(data){
			if(data.success){
	...
	function changeEdit(){
		...
		$("#Graduation").replaceWith('<input id="Graduation" class="inputtext-new" type="text"  name="info.graduationdate" onClick="WdatePicker()"/>');
