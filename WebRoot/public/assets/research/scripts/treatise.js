$(function() {
	...
	function getList(){		
		var searchCondtion = "?name=" + $.trim($("#name").val())+"&authors=" + $.trim($("#authors").val())+"&year=" + $.trim($("#year").val()),
			url = "/list/getTreatiseList" + encodeURI(searchCondtion);
	  T({
		 data : {
			pagehref : url,
			pagesize : 15,
		...
		rowList : [{
			...
			value : function(record, option, data){
				return '<a href="/research/viewtreatise?id=' + record.id
...
//		},
		{
			name : '操作',
			value : function(record, option, data){
				return reHtml = '<a href="/research/addtreatise?id=' + record.id
	...
	//搜索
  	...
  	$("#btn_add").click(function(){
  		window.location.href="/research/addtreatise";
  	});
  	//删除
  	...
  	$("#creatExcel").live("click",function(){
		...
		$.post("/list/getTreatiseexcel",{name:name,authors:authors,year:year},function(data){
			$.closeLoading();
			if(data.success){
				url = data.url;
