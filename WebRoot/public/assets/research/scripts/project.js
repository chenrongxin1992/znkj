$(function() {
	...
	function getList(){
		var searchCondtion = "?name=" + $.trim($("#name").val())+"&principal=" + $.trim($("#authors").val())+"&year=" + $.trim($("#year").val()),
			url = "/list/getProjectList" + encodeURI(searchCondtion);
	  T({
		 data : {
			pagehref : url,
			pagesize : 15,
		...
		rowList : [{
			...
			value : function(record, option, data){
				return '<a href="/research/viewproject?id=' + record.id
			...
			name : '操作',
			value : function(record, option, data){
				return reHtml = '<a  href="/research/addproject?id=' + record.id
	...
	//搜索
  	...
  	$("#btn_add").click(function(){
  		window.location.href="/research/addproject";
  	});
  	//删除
  	...
  	$("#creatExcel").live("click",function(){
  		...
  		$.post("/list/getProjectListExcel",{name:name,principal:principal,year:year},function(data){
  			$.closeLoading();
  			if(data.success){
  				url = data.url;
