(function(){
	...
	$(function(){
		...
		$("#add_relevance").live('click', function(){
			relnum++;
			var str = '<div id="relevance'+relnum+'"><select class="relevance" id="selrel'+relnum+'" onchange="nosysrelshow('+relnum+')">'+html+'</select>';
				...
				if(data.relevance!=""&&data.relevance!=null){
					...
					if(relevance.length>=1){
						for(var i=0, len=relevance.length; i<len; i++){
							relnum++;
							var str = '<div id="relevance'+relnum+'"><select class="relevance" id="selrel'+relnum+'" onchange="nosysrelshow('+relnum+')" value="'+relevance[i]+'">'+html+'</select>';
							if(relevance[i]!=-1){
								str += '<input type="text" class="input-normal selrel'+relnum+'"  style="display:none" />';
							}else{
...
//		tutors=tutors.substring(0,tutors.length-1);
		...
		$.post("/add/saveOrUpdateThesis",formdata,function(data){
			if(data.success){
				$.openSuccess('保存成功', 2000, function(){window.location.href = "/manage/editthesis?id="+data.id;});
...
//				for(var i=0; i<data.length; i++){
//					html += '<tr class="T-even-tr">';
//						html +='<td class="T-td-center"><div class="T-td-wrap T-no-break"><input type="checkbox" class="td-check" name="achid" value="'+data[i].id+'"/>'+n+'</div></td>'
//						     + '<td class="T-td-center"><div class="T-td-wrap T-no-break">'+data[i].name+'</div></td>'
//						     + '</tr>';
//						n++;
//				}
//			}else{
//				html += '<tr class="T-even-tr"><td class="T-td-center" colspan="3"><div class="T-td-wrap T-no-break" style="text-align: center;">暂无人员</div></td>';
//			}
//			html += '</tbody></table>';
//			dialog(html);
//			//设置checkbox选中状态
