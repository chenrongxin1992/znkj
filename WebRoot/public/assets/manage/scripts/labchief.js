$(function() {
	...
	function getList(){
		var searchCondtion = "?name=" + $.trim($("#name").val()),
			url = "/list/getLabChiefList" + encodeURI(searchCondtion);
	  T({
		 data : {
			pagehref : url,
			pagesize : 15,
		...
		rowList : [{
			...
			value : function(record, option, data){
				return '<a href="/manage/viewchief?id=' + record.id
			...
			name : '操作',
			value : function(record, option, data){
				return reHtml = '<a href="/manage/addchief?id=' + record.id
...
//新增
  	$("#btn_add").click(function(){
  		window.location.href="/manage/addchief";
  	});
//删除
  	$(".delete").live('click',function(){
  		if(confirm("确定要删除该记录吗？")){
	  		$.post("/delete/deleteLabModel?tkey=laboratory_chief&id="+ $(this).attr("id") , function(data){
				if (!data.success){
