$(function() {
	...
	function getList(){
		var searchCondtion = "?describe=" + $.trim($("#describe").val()),
		url = "/list/getIndexPicList" + encodeURI(searchCondtion);
	  T({
		 data : {
			pagehref : url,
			pagesize : 15,
		...
		rowList : [{
			...
			value : function(record, option, data){
				return reHtml = '<a  href="/manage/addindexpic?id=' + record.id
...
//新增
  	$("#btn_add").click(function(){
  		window.location.href="/manage/addindexpic";
  	});
//删除
  	$(".delete").live('click',function(){
  	   if(confirm("确定要删除该记录吗？")){
  		 $.post("/delete/?id="+ $(this).attr("id") , function(data){
 			if (!data.success){
