$(function() {
	...
	function getList(){		
		var searchCondtion = "?name=" + $.trim($("#name").val())+"&authors=" + $.trim($("#authors").val())+"&year=" + $.trim($("#year").val()),
			url = "/list/getLabAwardList" + encodeURI(searchCondtion);
	  T({
		 data : {
			pagehref : url,
			pagesize : 15,
		...
		rowList : [{
			...
			value : function(record, option, data){
				return '<a href="/manage/viewaward?id=' + record.id
			...
			name : '操作',
			value : function(record, option, data){
				return reHtml = '<a  href="/manage/editaward?id=' + record.id
   ...
   //删除记录
  	...
  	$("#creatExcel").live("click",function(){
  		var url="",
  		    ...
  		    year = $.trim($("#year").val());
  		$.openLoading('正在生成表格');
  		$.post("/list/getLabAwardexcel",{name:name,authors:authors,year:year},function(data){
  			$.closeLoading();
  			if(data.success){
  				url = data.url;
