$(function() {
	...
	function getList(){		
		var searchCondtion = "?name=" + $.trim($("#name").val())+"&authors=" + $.trim($("#authors").val())+"&year=" + $.trim($("#year").val()),
			url = "/list/getLabThesisList" + encodeURI(searchCondtion);
	  T({
		 data : {
			pagehref : url,
			pagesize : 15,
		...
		rowList : [{
			...
			value : function(record, option, data){
				return '<a href="/manage/viewthesis?id=' + record.id
			...
			name : '操作',
			value : function(record, option, data){
				return reHtml = '<a  href="/manage/editthesis?id=' + record.id
	...
	//下载表格
  	$("#creatExcel").live("click",function(){
  		var url="",
  		    ...
  		    year = $.trim($("#year").val());
  		$.post("/list/getLabThesisa",{name:name,authors:authors,year:year},function(data){
  			if(data.success){
  				url = data.url;
