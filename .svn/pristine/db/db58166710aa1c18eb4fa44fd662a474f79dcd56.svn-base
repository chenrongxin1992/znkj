$(function() {
	...
	function getList(){		
		var searchCondtion = "?name=" + $.trim($("#name").val())+"&authors=" + $.trim($("#authors").val())+"&year=" + $.trim($("#year").val()),
			url = "/list/getLabPerArticleList" + encodeURI(searchCondtion);
	  T({
		 data : {
			pagehref : url,
			pagesize : 15,
		...
		rowList : [{
			...
			value : function(record, option, data){
				return '<a href="/manage/viewperiodical?id=' + record.id
			...
			name : '操作',
			value : function(record, option, data){
				return reHtml = '<a  href="/manage/editperiodical?id=' + record.id
	...
	//搜索
  	...
  	$("#creatExcel").live("click",function(){
		...
		$.post("/list/getLabPerarticle",{name:name,authors:authors,year:year},function(data){
			$.closeLoading();
			if(data.success){
				url = data.url;
